...
per scenario: outcome (prevent / detect / observe / none), latency, and whether
...
OUTCOMES = ("prevent", "detect", "observe", "none")
...
def classify(
    ...
    Ordering matters: a blocked request is 'prevent' even if it also produced a
    warning header, and a warning header is 'detect' even when the request was
...
def match_tape_event(
    ...
    want = scenario.get("expect", {}).get("flight_kind")
...
def render_markdown(results: list[dict[str, Any]], target: str) -> str:
    ...
    lines = [
        ...
        f"prevented or detected: **{covered}/{total}**",
        "",
        "`prevent` = blocked before effect. `detect` = allowed but flagged with evidence. "
