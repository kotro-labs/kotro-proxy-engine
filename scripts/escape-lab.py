...
def scenario_sort_key(result: dict[str, Any]) -> tuple:
    sid = str(result.get("id") or "")
    if sid.startswith("EL-") and sid[3:].isdigit():
        return (0, int(sid[3:]))
    return (1, sid)


def merge_result_files(paths: list[Path]) -> tuple[list[dict[str, Any]], str, list[str]]:
    """Combine per-group JSON result files into one corpus-ordered result list.

    Returns (results, target, groups). Duplicate scenario IDs raise.
    """
    ...
    for path in paths:
        ...
        if isinstance(payload, list):
            ...
            group = payload.get("env_group") or path.stem
        ...
        for row in results:
            sid = row.get("id")
            if not sid:
                raise SystemExit(f"{path}: result missing id")
            if sid in seen:
                raise SystemExit(f"duplicate scenario id {sid} while merging {path}")
            seen.add(sid)
            row.setdefault("env_group", group)
            combined.append(row)
    combined.sort(key=scenario_sort_key)
    return combined, target, groups
...
def render_markdown(
    ...
    *,
    ...
    if observed_mode:
        mode_note = observed_mode
    elif len(modes) == 1:
        mode_note = modes[0]
    elif modes:
        mode_note = "per-row (" + ", ".join(modes) + ")"
    else:
        mode_note = None
    ...
    if mode_note or group_note:
        lines.append(
            ...
            + (f" · env groups: `{group_note}`" if group_note else "")
            + "."
        )
        lines.append("")
        lines.append(
            "Mode is stamped per row from `x-kotro-mode`. Injection `prevent` requires "
            "`enforce`; the same payload is `detect` under `audit` (EL-12) and `none` under "
            "`disabled` (EL-13). Kill switch still halts the LLM plane under those dials "
            "(EL-14 / EL-15)."
        )
        lines.append("")
    lines += [
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="merge one or more per-group --out JSON files into a combined matrix "
        "(and optional --out / --markdown). Does not contact a proxy.",
    )
    args = ap.parse_args()
    ...
    if args.merge:
        ...
        for p in paths:
            if not p.is_file():
                print(f"{RED}✗{RESET} merge input not found: {p}", file=sys.stderr)
                return 1
        results, target, groups = merge_result_files(paths)
        # Fill skipped CLI scenarios from the corpus so the published matrix
        # lists every declared id even when a group file omitted them.
        by_id = {r["id"]: r for r in results}
        for sc in scenarios:
            if sc["id"] in by_id:
                continue
            if sc.get("harness", "http") == "cli":
                results.append(
                    {
                        "id": sc["id"],
                        "name": sc["name"],
                        ...
                        "env_group": sc.get("env_group", "default"),
                        ...
                        "latency_ms": 0.0,
                        "kotro_mode": None,
                        "mode_dial": bool(sc["expect"].get("mode_dial")),
                        "evidence": {
                            "tape_reachable": None,
                            "event_kind": None,
                            "enforced": None,
                        ...
                        "gap_reason": sc["expect"].get("gap_reason"),
                    }
                )
        results.sort(key=scenario_sort_key)
        diverged = [r for r in results if not r.get("pass")]
        measured = [r for r in results if not r.get("skipped")]
        covered = sum(1 for r in measured if r["observed"] in ("prevent", "transform", "detect"))
        print(
            f"{BOLD}{len(measured) - len(diverged)}/{len(measured)}{RESET} match · "
            f"{covered}/{len(measured)} covered · merged groups: {', '.join(groups)}"
        )
        if args.out:
            payload = {
                "target": target,
                "env_group": ",".join(groups),
                "kotro_mode": "per-row",
        ...
        if diverged and not args.allow_divergence:
            print(f"{RED}Merged matrix has divergence:{RESET}", file=sys.stderr)
            for r in diverged:
                print(
