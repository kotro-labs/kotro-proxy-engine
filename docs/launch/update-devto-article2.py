#!/usr/bin/env python3
"""
Run this from your terminal to update the '5 Reasons' dev.to article.
Usage:  python3 update-devto-article2.py
"""
...
target = next(
    (a for a in articles if "5-reasons" in a["slug"]),
    None
)
if not target:
    print("Article not found. Articles on your account:")
    for a in articles:
        print(" ", a["id"], a["slug"])
    raise SystemExit(1)

article_id = target["id"]
print(f"Found article: id={article_id}  slug={target['slug']}")
...
payload = json.dumps({"article": {"body_markdown": UPDATED_BODY}}).encode()
